...
void GPIO_LedOff(void) {
    GPIO_WriteBit(LED_PORT, LED_PIN, Bit_SET); //inverse control
}

void GPIO_LedOn(void) {
    GPIO_WriteBit(LED_PORT, LED_PIN, Bit_RESET); //inverse control
}

bool GPIO_LedIsOn(void) {
    return !(bool) GPIO_ReadInputDataBit(LED_PORT, LED_PIN);
}
